...
def write_fake_rclone(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    script.write_text(
        ...
        "tar_date = os.environ.get('FAKE_RCLONE_TAR_DATE', date)\n"
        "tar_name = os.environ.get('FAKE_RCLONE_TAR_NAME', f'hermes-fleet-encrypted-{tar_date}.tar')\n"
        "if cmd == 'lsd':\n"
        ...
        "    if tar_date not in ('2026-06-01', date):\n"
        "        print(f'          -1 {tar_date} 00:00:00        -1 {tar_date}')\n"
        "    print(f'          -1 {date} 00:00:00        -1 {date}')\n"
        "    sys.exit(0)\n"
        "if cmd == 'lsf':\n"
        "    mode = os.environ.get('FAKE_RCLONE_MODE', 'ok')\n"
        "    remote = args[1] if len(args) > 1 else ''\n"
        "    if mode == 'notar' or not (remote.endswith(tar_date) or ('/' + tar_date) in remote):\n"
        "        print('notes.txt')\n"
        "    else:\n"
        "        print(tar_name)\n"
        "    sys.exit(0)\n"
...
def test_latest_tar_discovery_skips_newest_date_without_tar(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FAKE_RCLONE_TAR_DATE", "2026-07-01")
    monkeypatch.setenv("FAKE_RCLONE_BYTES", "2048")
    target = check_target(cfg(tmp_path, rclone=rclone, min_bytes=1024))
    assert target.remote_path == "fleet-offsite2:/2026-07-01/hermes-fleet-encrypted-2026-07-01.tar"
...
def test_run_writes_offsite_proof_with_stubbed_drill_and_fake_rclone(tmp_path, monkeypatch):
    ...
    assert proof["size"] == source_tar.stat().st_size
